...
class Model(object):
    ...
    def __init__(self):
        ...
        self.kernel =Matern(length_scale=0.03, nu=1.5) + WhiteKernel(noise_level=2.438)
        self.gpr = GaussianProcessRegressor(kernel=self.kernel, alpha=1e-10, n_restarts_optimizer=0)
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        self.gpr.fit(train_x[:5000, :], train_y[:5000])
        # print(self.gpr.kernel_)
        # print(self.gpr.kernel_.theta)
