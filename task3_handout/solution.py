...
class BO_algo(object):
    def __init__(self):
        ...
        self.previous_points = np.array([[2.0], [1.0], [1.5], [2.3]])
        ...
        self.constraint_model = GaussianProcessRegressor()  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor()  # TODO : GP model for your acquisition function
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        #print("input of acquisition function: ",x)
        #mu, sigma = self.objective_model.predict(x, return_std=True)
        #mu_sample = self.objective_model.predict(np.array(self.previous_points))
        #x_sample = np.array([self.previous_points[0,:],self.previous_points[1,:]])
        #mu_sample = self.objective_model.predict(x_sample)
        ...
        #    pos=[(ix,iy) for ix, row in enumerate(sigma) for iy, i in enumerate(row) if i == 0]
        #    ei[pos] = 0.0
        #return ei[0][0]
        
        mean_y_new, sigma_y_new = self.objective_model.predict(x.reshape(-1,1), return_std=True)
        sigma_y_new = sigma_y_new.reshape(-1,1)
        
        # Using estimates from Gaussian surrogate instead of actual function for entire prior distribution to avoid cost
        x_sample = np.array([self.previous_points[0,:],self.previous_points[1,:]])
        mean_y = self.objective_model.predict(x_sample.reshape(-1,1))
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        """
        print("before add: ",self.previous_points,x[:,0],x[:,1],z,c)

        self.previous_points=np.concatenate((self.previous_points,np.array([x[:,0],x[:,1],z,c])), axis=1)
        print("after add: ", self.previous_points)
        print("x sample:", np.array([self.previous_points[0,:],self.previous_points[1,:]]))
    ...
    def get_solution(self) -> np.ndarray:
        ...
        x_opt = np.array([self.previous_points[-1,0],self.previous_points[-1,1]])
        return x_opt
...
def train_on_toy(agent, iteration):
    ...
    print(" the soultion we get in train_on_toy: ", solution)
    print("domain shape", domain_x.shape[0])
