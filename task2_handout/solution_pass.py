...
class Model(object):
    ...
    def __init__(self):
        ...
        hidden_layers = (128)  # for each entry, creates a hidden layer with the corresponding number of units
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = spike_slab_2GMM(mu1=0, mu2=0, sigma1=0.135, sigma2=0.0001, pi=0.75)
        # self.prior = spike_slab_2GMM(mu1=0, mu2=0, sigma1=np.sqrt(2)/in_features, sigma2=np.sqrt(0.5)/in_features, pi=0.5)
        
        # Learnable parameters
        self.W_mu = nn.Parameter(torch.Tensor(self.in_features, self.out_features).uniform_(-0.2, 0.2))
        self.W_p = nn.Parameter(torch.Tensor(self.in_features, self.out_features).uniform_(-5, -1)) #rho

        self.b_mu = nn.Parameter(torch.Tensor(self.out_features).uniform_(-0.2, 0.2))
        self.b_p = nn.Parameter(torch.Tensor(self.out_features).uniform_(-5, -1)) #rho
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = Variable(self.b_mu.data.new(self.b_mu.size()).normal_())
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias:
            eps_W = Variable(self.W_mu.data.new(self.W_mu.size()).normal_()) # N(0,1), same size as W_mu
            eps_b = Variable(self.b_mu.data.new(self.b_mu.size()).normal_())
            
            
            std_w = 1e-6 + F.softplus(self.W_p, beta=1, threshold=20) #convert rho to std
        ...
        log_prior = self.prior.loglike(W) + self.prior.loglike(b)
    
        log_variational_posterior = isotropic_gauss_loglike(W, self.W_mu, std_w) + isotropic_gauss_loglike(b, self.b_mu, std_b)

        weights = W
        bias = b
        output = torch.mm(inputs, W) + b.unsqueeze(0).expand(inputs.shape[0], -1)
