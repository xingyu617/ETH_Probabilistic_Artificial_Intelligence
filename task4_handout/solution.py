...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        with torch.no_grad():
            action,log_pa=self.pi.forward(state)
            act=action.sample()
            val=self.v.forward(state)

        return act.item(),val.item(),action.log_prob(act)
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        V_st_next = vals[1:]
        V_st = vals[:-1]
        deltas = rews[:-1] + self.gamma*vals[1:] - V_st
    ...
    def get(self):
        ...
        data = dict(obs=self.obs_buf, act=self.act_buf, ret=self.ret_buf,
                    phi=self.phi_buf, logp=self.logp_buf,value=self.val_buf)
...
class Agent:
    ...
    def get_action(self, obs):
        ...
        return self.ac.step(torch.Tensor(obs))[0]
